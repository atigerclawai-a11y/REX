...
# Load Instagram + other Meta credentials from the hermes-cloud env file.
# REX's main.py does not load this file at startup, so do it here at import time.
try:
    from dotenv import load_dotenv as _load_dotenv
    _load_dotenv(os.path.expanduser("~/.hermes-cloud/.env"), override=False)
except Exception:
    pass
...
PLATFORMS: Dict[str, Dict[str, Any]] = {
    "instagram": {
        ...
        "configured": True,
        "autopost_ready": True,         # container→poll→publish flow wired in execute_post
        "mcp_server": "instagram",
        "notes": "META_IG_ACCESS_TOKEN in Hermes env. Posts via Facebook Graph API directly (post_instagram helper). Drafts still require explicit /approve before /execute.",
    },
    "telegram": {
...
class DraftRequest(BaseModel):
    ...
    image_url: Optional[str] = None       # required for image-platforms (instagram)
...
async def post_instagram(media_url: str, caption: str) -> Dict[str, Any]:
    """Post an image or video to @boardwalkbeergarden via the Facebook Graph API.

    Detects video by .mp4 extension — uses video_url + media_type=REELS for videos.
    Three-step flow:
    ...
    Env vars (loaded from ~/.hermes-cloud/.env at module import):
      META_IG_ACCESS_TOKEN, META_IG_USER_ID
    """
    token = os.getenv("META_IG_ACCESS_TOKEN")
    ig_user_id = os.getenv("META_IG_USER_ID", "27923669980556036")
    if not token:
        return {"error": "META_IG_ACCESS_TOKEN not set"}

    is_video = media_url.lower().endswith(".mp4") or media_url.lower().endswith(".mov")
    ...
    async with httpx.AsyncClient(timeout=60.0) as client:
        ...
        if not container_id:
            return {"error": "Container creation failed", "status": r.status_code, "detail": r.text[:500]}

        # Step 2: Poll status (max ~60s for video, ~30s for image)
        max_polls = 20 if is_video else 10
...
async def create_draft(req: DraftRequest):
    ...
    for platform_key in req.platforms:
        ...
        draft = {
            ...
            "content": content,
            "image_url": req.image_url,           # only used by image-platforms (instagram)
...
async def execute_post(draft_id: str):
    ...
    # Instagram: container → poll → publish via Facebook Graph API
    if platform == "instagram":
        image_url = draft.get("image_url")
        if not image_url:
            raise HTTPException(
                status_code=400,
                detail="Instagram drafts require image_url. Recreate the draft with an image_url field.",
            )
        result = await post_instagram(media_url=image_url, caption=draft["content"])
        if result.get("error") or not result.get("id"):
            logger.error(f"Instagram post failed: {result}")
            raise HTTPException(status_code=502, detail=f"Instagram post failed: {result}")
        draft["ig_media_id"] = result["id"]
