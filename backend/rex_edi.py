...
def parse_837(raw: str) -> Dict[str, Any]:
    ...
    for seg in segments:
        ...
        if tag == "ISA":
            ...
            elif nm1_type == "IL" and current_claim:   # Subscriber
                current_claim["subscriber_last"]  = _el(seg, 3)
                current_claim["subscriber_first"] = _el(seg, 4)
                current_claim["subscriber_id"]    = _el(seg, 9)
                current_claim["payer_id"]         = ""
            elif nm1_type == "PR" and current_claim:   # Payer
                current_claim["payer_name"] = _el(seg, 3)
                current_claim["payer_id"]   = _el(seg, 9)
...
if __name__ == "__main__":
    import sys
    if len(sys.argv) < 2:
        print("Usage: python rex_edi.py <file.835|file.837>")
        sys.exit(1)
