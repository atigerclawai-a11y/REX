#!/usr/bin/env python3
"""GOJ Wednesday blank menus SPLIT BY SHIFT — one PDF per shift.
Wed S1 (73) / Wed S2 (95). QR bottom-right clear band, [ID] in footer."""
...
rosters = json.load(open('/tmp/wed_definitive.json'))  # s1/s2 auth-format names

auth = sqlite3.connect('/Users/mainsobhelper/Documents/goj files/dashboard/auth_tracker.db')
# Roster = auth day_W_actual (same source as sheets) → guarantees menu count = sheet count.
...
build(s2_roster, f"{OUT}/Menus_Wed_Aug05_S2_LIVE.pdf", "GoJ Wed Aug 5 SHIFT 2 menus (95)")
