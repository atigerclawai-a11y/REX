#!/usr/bin/env python3
"""Check Fedorova's THURSDAY order history (pre-Aug, before house_standard)."""
import sqlite3

p = sqlite3.connect('/Users/mainsobhelper/Documents/goj files/proprietary/goj_proprietary.db')
print('=== Fedorova Olga ALL Thursday rows (ever) ===')
for r in p.execute("""SELECT menu_date, day_code, shift, salad, soup, main, side, source_sheet
    FROM client_menus WHERE client_name='Fedorova Olga' AND day_code='TH'
    ORDER BY menu_date DESC LIMIT 10"""):
    print(f'  {r[0]} {r[1]} S{r[2]}: {r[3]}|{r[4]}|{r[5]}|{r[6]} [{r[7]}]')

print('\n=== Fedorova ALL rows July (any day) ===')
for r in p.execute("""SELECT menu_date, day_code, shift, salad, soup, main, side, source_sheet
    FROM client_menus WHERE client_name='Fedorova Olga' AND menu_date BETWEEN '2026-07-01' AND '2026-08-02'
    ORDER BY menu_date DESC LIMIT 15"""):
    print(f'  {r[0]} {r[1]} S{r[2]}: {r[3]}|{r[4]}|{r[5]}|{r[6]} [{r[7]}]')
p.close()
