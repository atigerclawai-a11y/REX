#!/usr/bin/env python3
"""GOJ Tuesday blank menus SPLIT BY SHIFT — one PDF per shift.
Tue S1 (81) / Tue S2 (55). QR bottom-right clear band, [ID] in footer.
Roster = auth day_T_actual (same source as sheets) → count guaranteed."""
...
# Roster = auth day_T_actual (same source as sheets) → guarantees menu count = sheet count.
s1_roster = [r[0] for r in auth.execute("SELECT name FROM clients WHERE active=1 AND day_T_actual=1 ORDER BY name")]
s2_roster = [r[0] for r in auth.execute("SELECT name FROM clients WHERE active=1 AND day_T_actual=2 ORDER BY name")]
id_by_name = {}
...
def _flip(name):
    """'LAST, FIRST' (Carecenta) → 'last first' (auth format: Last First)."""
...
build(s1_roster, f"{OUT}/Menus_Tue_Aug04_S1_LIVE.pdf", "GoJ Tue Aug 4 SHIFT 1 menus (81)")
build(s2_roster, f"{OUT}/Menus_Tue_Aug04_S2_LIVE.pdf", "GoJ Tue Aug 4 SHIFT 2 menus (55)")
