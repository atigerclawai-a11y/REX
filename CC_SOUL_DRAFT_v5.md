...
# v5.1 · May 31 2026 · Full architecture + GOJ multi-agent system + cascade rules
...
| Fallback 1 | `claude-sonnet-4-6` | Anthropic |
...
| Anthropic | claude-sonnet-4-6, claude-opus-4-6, claude-haiku-4-5 | Fallback 1 |
| Google | gemini-2.0-flash, gemini-2.5-flash, gemini-2.5-pro | Fallback 2 |
...
| OpenRouter | router only — non-DeepSeek routing | Available |
| Ollama (local) | qwen3:14b (default), llama3.1:8b | :11434 |
...
| Hermes local gateway | 3001 | **BROKEN — crashes on start. Root cause unknown.** |
...
**Local inference:**
- Ollama :11434 — qwen3:14b (default), llama3.1:8b
...
**NOTE — ShellCore vs. this system:**
**ShellCore** is a SEPARATE earlier project — Kato's pre-Jarvis prototype attempt, built around the same time as the Claus/Hermes work. ShellCore was an early attempt at what Jarvis (Phase 19 HUD) would become. The Tauri native app was part of that vision (Kato wanted a native phone + desktop app), but was shelved as "too early." ShellCore may have partial Phase 1 code (`ShellCore_Phase1_Closeout_Report.md` exists — FastAPI on :8081, JSON-backed local registry). ShellCore ≠ the 13-agent planned system below. Clarify with Hermes if uncertain.
...
- **Emergent AI** — noted as product of interest (§14.7). Must be evaluated against local-first posture before any integration.
...
| 13 | **Luna** (Child Companion) | For Chairman's 9-year-old daughter. Permanent baseline — NEVER promotes. Hardcoded refusals, time caps, full visibility to Chairman. Last to activate. Requires exhaustive Officer Riggs red-team review. | 2 | Pink |
...
| **TransitionAgent** | ✅ Running | HIGHEST PRIORITY. `com.goj.transition-agent.plist` active. Google Drive monitoring hook NOT built. 2-week window. |
...
**Fireflies:** Wired MCP connector. Use case confirmed: **meeting/voice chat transcription → organized documentation → Obsidian vault.** Audit meetings, strategy sessions, voice chats → Fireflies captures + transcribes → structured docs land in Obsidian. Build-and-drift pattern — integrated, then drifted inactive. API key likely in Keychain. Check Keychain before assuming unavailable.

**Palace (Mem Palace):** Downloaded from GitHub. `palace_main.db` (144KB) and `palace_cloud.db` (24KB) on external drive. A memory/knowledge system. NOT a legacy artifact.
...
2. **iMessage watcher** — monitoring 3 work group chats for schedule changes (someone changing their day, calling in sick, won't be there tomorrow)
...
**CRITICAL — Schedule Change Cascade:**
When someone changes their day, is sick, or won't be there tomorrow — it propagates to ALL of these:
- **Calendar** — schedule updated
- **Attendance records** — attendance log modified
- **Driver's list** — route adjusted (remove/add client)
- **Kitchen's list** — meal count adjusted (more or fewer of that client's preferences)
- **Distribution logs** — distribution count updated
- **Sign-in sheets** — expected attendance updated
- **Menus** — that client's individual menu for the day adjusted

The iMessage watcher is the input; the cascade is the output. No partial updates — all systems must reflect the change.
...
| # | Item | Status |
|---|------|--------|
| 1 | **TransitionAgent** — Google Drive monitoring hook NOT built | HIGHEST PRIORITY · 2-week window |
| 2 | **config.yaml line 18 syntax error** — `system_prompt` not loading; gateway uses fallback | Active bug |
...
| 8 | **iMessage watcher** — 3 work group chats — reportedly live but unverified (system hasn't been stable enough to confirm it's doing anything) | Verify when system is healthy |
| 9 | **Fireflies** — API key likely in Keychain; integration wired but drifted inactive | Check Keychain, wire transcript → Obsidian flow |
